...
`The useRouteMatch hook attempts to match the current URL in the same way that a <Route> would. It's useful for getting access to the match data without actually rendering a <Route> .`

2. How would you explain form validation to someone who has never programmed before?

`To increase accessibility by making sure that the right data is typed in the right fields, so that way there is clear communication all around.`

3. In 1-2 sentences, define end to end testing. 

`End to End testing also known as E2E looks at an entire user experience. It makes sure the desired action can be acomplished.`
